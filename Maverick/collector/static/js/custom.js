...
$('#btn_newPersona,#generar').click(function (e) {
    e.preventDefault();
    newPersona();
});
newPersona = function () {
    $("#newPersona #validar").html('');
    $('#newPersona').modal('show');
    $.ajax({
        type: 'GET',
        url: '/obstetrica/generar/',
        data: 'json',
        success: function (data) {
            $("#form_persona input[name=anterior]").val(parseInt(data.anterior__max )+1);
        },
        error: function (xhr, ajaxOptions, thrownError) {
            $("#newPersona").find('#validar').append('<li>error ' + xhr.status +' al obtener N° H.C.</li>');
        }
    });
};
